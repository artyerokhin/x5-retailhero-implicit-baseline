...
with open("x5_implicit.pkl", "rb") as f:
    model, client_dict, reverse_client_dict, \
                 product_dict, reverse_product_dict = pickle.load(f)
...
def recommend():
    try:
        ...
        return jsonify({
            'recommended_products': [pred[1] for pred in predict_user(model,
                user_id, products, product_dict, reverse_product_dict, matrix_shape)]})
